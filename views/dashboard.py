...
def render_dashboard():
    ...
    with c2:
        st.markdown(f"<div style='text-align:right; font-family:monospace; color:#52525B;'>AI ENGINE: V4.2</div>", unsafe_allow_html=True)
    ...
    with col_chat:
        ...
        if prompt := st.chat_input("Bir talimat verin (Örn: Stok analizi)..."):
            ...
            # 3. CEVAP ÜRET (Brain.py'den çekiyoruz!)
            full_response = ""
            # Streamlit native write yerine string biriktirip tek seferde basıyoruz
            try:
                for chunk in brain.get_streaming_response(st.session_state.messages, st.session_state.user_data):
                    full_response += chunk
            except:
                full_response = "Bağlantı sağlanamadı, ancak verileriniz güvende."

            st.session_state.messages.append({"role": "assistant", "content": full_response})
            
            # 4. Sayfayı Yenile (Görsel ve Chat Güncellensin)
            st.rerun()
